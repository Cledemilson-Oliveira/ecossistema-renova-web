...
const CLOSED=new Set(['concluida','concluido','concluidas','concluidos','cancelada','cancelado','canceladas','cancelados','fechada','fechado','fechadas','fechados','finalizada','finalizado','finalizadas','finalizados']);
const SALE_CLOSED=new Set(['fechada','fechado','pago','paga','aprovado','approved']);
const APPOINTMENT_CLOSED=new Set(['cancelado','cancelada','concluido','concluida','realizado','realizada','recusado','recusada']);
...
function ensureStyles(){if(document.getElementById('renovaDashboardCss'))return;const l=document.createElement('link');l.id='renovaDashboardCss';l.rel='stylesheet';l.href='./css/dashboard.css?v=20260914-005';document.head.appendChild(l)}
...
<div class="dashboard-metrics">
 ...
 <article class="dashboard-metric positive"><span class="metric-label">Clientes</span><strong id="dashClients">0</strong><small>CRM + compradores já convertidos</small></article>
 ...
 <article class="dashboard-metric"><span class="metric-label">Ticket médio</span><strong id="dashAverageTicket">R$ 0,00</strong><small>Vendas fechadas do mês</small></article>
</div>
<div class="dashboard-columns">
 ...
 <section class="dashboard-panel"><div class="panel-heading"><div><span class="eyebrow">OPERAÇÃO</span><h3>Atenção agora</h3></div></div><div class="operation-stack">
  ...
  <div class="operation-row"><span>Tarefas com prazo abertas</span><strong id="dashOpenTasks">0</strong></div>
  <div class="operation-row"><span>Tarefas atrasadas</span><strong id="dashOverdueTasks">0</strong></div>
  <div class="operation-row"><span>Comissões pendentes</span><strong id="dashPendingCommissions">R$ 0,00</strong></div>
 ...
 </div><p class="operation-note">Tarefas sem prazo continuam no módulo Tarefas, mas não poluem o bloco de atenção. Pedido pago vira Venda e Receita automaticamente.</p></section>
</div>`}
...
export async function loadDashboard({supabase,company,profile}){
 ...
 const [contactsRes,salesRes,financeRes,tasksRes,serviceOrdersRes,appointmentsRes,pedidosRes,commissionsRes]=await Promise.all([
  supabase.from('contatos').select('id,nome,tipo,etapa,origem,criado_em').eq('empresa_id',companyId),
  supabase.from('vendas').select('id,contato_id,cliente_nome,produto_nome,produto,valor,status,data_venda,criado_em,origem').eq('empresa_id',companyId),
  ...
  supabase.from('comissoes').select('id,valor_comissao,status,gerada_em,paga_em').eq('empresa_id',companyId)
 ]);
 const responses=[contactsRes,salesRes,financeRes,tasksRes,serviceOrdersRes,appointmentsRes,pedidosRes,commissionsRes];const errors=responses.map(r=>r.error).filter(Boolean);if(errors.length){console.error('RENOVA dashboard',errors);setText('dashboardUpdated','Dados parcialmente disponíveis')}
 const contacts=contactsRes.data||[],sales=salesRes.data||[],finance=financeRes.data||[],tasks=tasksRes.data||[],serviceOrders=serviceOrdersRes.data||[],appointments=appointmentsRes.data||[],pedidos=pedidosRes.data||[],commissions=commissionsRes.data||[],now=new Date();
 const paidFinance=finance.filter(f=>key(f.status)==='pago');
 const monthRevenue=paidFinance.filter(f=>key(f.tipo)==='receita'&&sameMonth(dateOf(f,['data_pagamento','criado_em']),now)).reduce((s,f)=>s+n(f.valor),0);
 const monthExpense=paidFinance.filter(f=>key(f.tipo)==='despesa'&&sameMonth(dateOf(f,['data_pagamento','criado_em']),now)).reduce((s,f)=>s+n(f.valor),0);
 const monthSales=sales.filter(v=>SALE_CLOSED.has(key(v.status))&&sameMonth(dateOf(v,['data_venda','criado_em']),now));
 const monthSalesValue=monthSales.reduce((s,v)=>s+n(v.valor),0);
 const averageTicket=monthSales.length?monthSalesValue/monthSales.length:0;
 const crmClients=contacts.filter(c=>key(c.tipo)==='cliente');
 const leads=contacts.filter(c=>key(c.tipo)!=='cliente');
 const clientKeys=new Set(crmClients.map(c=>`crm:${c.id}`));
 for(const sale of sales.filter(v=>SALE_CLOSED.has(key(v.status)))){
   if(sale.contato_id){clientKeys.add(`crm:${sale.contato_id}`);continue}
   const name=key(sale.cliente_nome);if(name)clientKeys.add(`legacy:${name}`);
 }
 const monthAppointments=appointments.filter(a=>sameMonth(dateOf(a,['inicio_em','criado_em']),now));
 const upcomingAppointments=appointments.filter(a=>{const d=dateOf(a,['inicio_em']);return d&&d>=now&&!APPOINTMENT_CLOSED.has(key(a.status))});
 const monthPedidos=pedidos.filter(p=>sameMonth(dateOf(p,['criado_em']),now));
 const openPedidos=pedidos.filter(p=>!CLOSED.has(key(p.status)));
 const trulyOpenTasks=tasks.filter(t=>!CLOSED.has(key(t.status)));
 const datedOpenTasks=trulyOpenTasks.filter(t=>Boolean(dateOf(t,['data'])));
 const openOrders=serviceOrders.filter(o=>!CLOSED.has(key(o.status)));
 const today=new Date(now.getFullYear(),now.getMonth(),now.getDate());
 const overdue=datedOpenTasks.filter(t=>{const d=dateOf(t,['data']);return d&&d<today});
 const ownerAction=trulyOpenTasks.filter(t=>t.requer_acao_dono===true),blocked=trulyOpenTasks.filter(t=>t.bloqueada===true);
 const pendingCommissions=commissions.filter(c=>['pendente','pending','aguardando'].includes(key(c.status))).reduce((s,c)=>s+n(c.valor_comissao),0);
 setText('dashRevenue',money.format(monthRevenue));setText('dashSalesCount',String(monthSales.length));setText('dashSalesValue',money.format(monthSalesValue));setText('dashLeads',String(leads.length));setText('dashClients',String(clientKeys.size));setText('dashAppointments',String(monthAppointments.length));setText('dashUpcomingAppointments',`${upcomingAppointments.length} próximos`);setText('dashOrdersCount',String(monthPedidos.length));setText('dashOpenPedidos',`${openPedidos.length} em operação`);setText('dashOpenTasks',String(datedOpenTasks.length));setText('dashExpense',money.format(monthExpense));setText('dashBalance',money.format(monthRevenue-monthExpense));setText('dashAverageTicket',money.format(averageTicket));setText('dashOpenOrders',String(openOrders.length));setText('dashOverdueTasks',String(overdue.length));setText('dashPendingCommissions',money.format(pendingCommissions));setText('dashOwnerAction',String(ownerAction.length));setText('dashBlockedTasks',String(blocked.length));setText('dashboardUpdated',`Atualizado agora • ${company.nome||'Empresa'}`);
 const activities=[];
 finance.forEach(f=>activities.push({kind:key(f.tipo)==='receita'?'revenue':'expense',icon:key(f.tipo)==='receita'?'↗':'↘',title:f.descricao||f.tipo,detail:`${f.tipo} • ${f.status||''}${f.origem?` • ${f.origem}`:''}`,value:n(f.valor),date:dateOf(f,['data_pagamento','vencimento','criado_em'])}));
 ...
 trulyOpenTasks.forEach(t=>activities.push({kind:'task',icon:'▣',title:t.titulo||'Tarefa',detail:`${t.categoria||'Tarefa'} • ${t.status||''}`,date:dateOf(t,['data','criado_em'])}));
